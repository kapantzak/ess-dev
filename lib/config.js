...
const instructions = (outPath, essdevConfig) => {
  ...
  const obj = {
    ...
    models: [
      ...
      {
        template: path.join(templates, "model_datasources.cs"),
        output: formName => {
          const modelsFolderName = helpers.getModelsFolderName(formName);
          const fileName = "Datasources.cs";
          const treeObj = {
            classes: {
              HttpRequestsDataModels: {
                [modelsFolderName]: {
  ...
  if (storedProc) {
    ...
    if (returnData) {
      ...
      (returnData.data || {}).forEach(dt => {
        const name = dt.name;
        dataClassProps.push({
          name: dt.isPivot ? name : `${name}Collection`,
          type: dt.isPivot ? name : `List<${name}>`
        });
        const dataModel = {
          name,
          isPivot: dt.isPivot,
