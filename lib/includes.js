...
const includeUserControlHelperCompile = formName => {
  return includeItem("Compile", formName)(getUserControlHelperCompile);
};
...
const getUserControlHelperCompile = formName => {
  const fileName = helpers.getUserControlHelperFileName(formName);
  return [
    {
      unevaluatedInclude: `classes\\UserControlHelpers\\${fileName}`
    }
  ];
};
...
const includeInProjectFile = (key, formName) => {
  switch (key) {
    ...
    case "userControlHelper":
      return includeUserControlHelperCompile(formName);
    default:
      return [];
  }
};
