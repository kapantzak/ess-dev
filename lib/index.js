...
const cmd_init = async argv => {
  ...
  if (answers && answers.confirmation) {
    ...
    let warnings = 0;
    const logs = [];
    let projectXmlJson = files.readProjectFile(config.projectFilePath);
    ...
    for await (const x of Object.entries(instructions)) {
      ...
      if (helpers.renderInstructionItem(key, answers)) {
        ...
        projectXmlJson = helpers.includeInProjectFile(
          key,
          projectXmlJson,
          formName
        );
    ...
    files.replaceFileWith(config.projectFilePath, newContents);
